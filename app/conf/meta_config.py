...
#
# 用途（组合关系）：
#   - 被 MetaKnowledgeService.build() 使用（见 services/meta_knowledge_service.py）
#   - 在应用启动时（core/lifespan.py）或命令行脚本（scripts/build_meta_knowledge.py）
#     中，依据 meta_config.yaml 将表/字段/指标定义同步到三大存储引擎：
